...
Currently, it only describes the serialization of OpenTelemetry data to the OTLP
JSON format.
...
This document describes the serialization of OpenTelemetry data as JSON objects
that can be stored in files.

### File storage requirements

#### JSON lines file

This file is a JSON lines file (jsonlines.org), and therefore follows those
requirements:
...
There is no guarantee in particular that timestamps will be monotonically
increasing.
...
Only top-level objects, `ExportTraceServiceRequest`,
`ExportMetricsServiceRequest`, and `ExportLogsServiceRequest` are supported.
