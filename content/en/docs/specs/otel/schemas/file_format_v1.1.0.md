...
This transformation splits a metric into several metrics and eliminates an
attribute. Here is the structure:
...
The current value for this setting is "1.1.0". Any change to this number MUST be
...
- MINOR number SHOULD be increased if a new setting is added to the file format
  ...
  - A new setting is added to an existing transformation. The new setting is
    optional and the default value of the setting matches the behavior of the
    previous file format version.
...
- MAJOR number SHOULD be increased if the file format is changed in an
  incompatible way. This means the consumers of the file need to parse or
  interpret the file differently compared to previous MAJOR version.
